...
void UpdateEnemy(void)
{
	...
	for (int nCntEnemy = 0; nCntEnemy < MAX_ENEMY; pEnemy++,nCntEnemy++)
	{
		if (!pEnemy->bUse)
		{
			continue;	//処理を１回飛ばす　[1]
...
Enemy* SetEnemy(D3DXVECTOR3 pos, ENEMYTYPE nType)
{
	...
	for (int nCntEnemy = 0; nCntEnemy < MAX_ENEMY; nCntEnemy++,pEnemy++)
	{
		...
		ZeroMemory(pEnemy, sizeof(s_aEnemy[0]));
...
void TrackingMove(Enemy * pEnemy)
{
	...
	{//true の時
		...
		pEnemy->move.x = (moveEnemyX / moveEnemyR) * MOVE_PEED;
		pEnemy->move.y = (moveEnemyY / moveEnemyR) * MOVE_PEED;
	}
}
...
void UpdateSky1(Enemy* pEnemy)
{
	...
	Collision(ENEMYTYPE_SKY_1, pEnemy);
...
void Collision(ENEMYTYPE nType, Enemy* pEnemy)
{
	Player* pPlayer = GetPlayer();
