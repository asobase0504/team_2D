...
//----------------------------
//static 変数
static LPDIRECT3DTEXTURE9 s_Texture[TYPEENEMY_MAX] = {};	//テクスチャへのポインタ
...
void UpdateEnemy(void)
{
	for (int nCntEnemy = 0; nCntEnemy < MAX_ENEMY; nCntEnemy++)
	{
		...
		if (pEnemy->nLife > 0)
		{
			...
			case TYPEENEMY_SKY_1:
				...
				{//敵が逃げる処理
					...
					pEnemy->flg = false;
					//g_aEnemy[nCntEnemy].move.y = 0;		//コメントアウトを消すと直角に曲がる
					pEnemy->move.x = 3;
			...
			case TYPEENEMY_SKY_3:		//引き返し敵
				...
				{// 敵が逃げる処理
					pEnemy->flg = false;		//追尾
					g_aEnemy[nCntEnemy].Buk = true;		//戻る処理
				}
				break;
		...
		pVtx += 4 * nCntBoss;	// 頂点座標データのポインタを４つ分進める		
		...
		if (g_nCounterAnimBoss >= 10)
		{
			...
			pVtx[0].tex = D3DXVECTOR2((1.0f / 12.0f) * g_nPatternanimBoss, 0.0f);			//--------------------------敵が増えたとき増やす
...
void SetEnemy(D3DXVECTOR3 pos, TYPEENEMY nType)
{
	for (int nCntBoss = 0; nCntBoss < MAX_ENEMY; nCntBoss++)
	{
		...
		pVtx += nCntBoss * 4;	// 頂点座標データのポインタを４つ分進める
...
void HitEnemy(int nCntBoss, int nDamage)
{
	...
	{
		float moveBossX = Player.x - pEnemy->pos.x;				// 自分と敵のｘを計算
		float moveBossY = Player.y - pEnemy->pos.y;				// 同様ｙ計算
		float moveBossR = sqrtf(moveBossX * moveBossX + moveBossY * moveBossY);

		pEnemy->move.x = -(moveBossX / moveBossR) * 30.0f;	// 死んだとき飛んでく
